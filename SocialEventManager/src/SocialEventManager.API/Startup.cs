...
namespace SocialEventManager.API {
  public class Startup {
    ...
    public void ConfigureServices(IServiceCollection services) {
      services
          .Configure<BasicAuthenticationConfiguration>(
              Configuration.GetSection(AuthConstants.BasicAuthentication))
      ...
      services
          .AddCors(options =>
                       options.AddPolicy(ApiConstants.AllowAll,
          ...
          .AddScheme<AuthenticationSchemeOptions, BasicAuthenticationHandler>(
              AuthConstants.AuthenticationScheme, null);
      ...
      services.AddSignalR().AddHubOptions<ChatHub>(
          options => options.AddFilter<ChatHubLogFilter>());
    ...
    Configure(IApplicationBuilder app, IWebHostEnvironment env,
      ...
      Task.Run(() =>
                   new DbMigrations(Configuration).Migrate(env.EnvironmentName))
          .GetAwaiter()
          .GetResult();

      app.UseApiExceptionHandler(options => {
           options.AddResponseDetails =
               ErrorResponseHandler.UpdateApiErrorResponse;
           options.DetermineLogLevel = ErrorResponseHandler.DetermineLogLevel;
         })
          .UseResponseCompression()

          // TODO: Currently, the Hangfire dashboard is opened to all users.
          // Need to implement an authorization scenario. A helpful link for
          // implementing it:
          // https://sahansera.dev/securing-hangfire-dashboard-with-endpoint-routing-auth-policy-aspnetcore/
          .UseHangfireDashboard(
              ApiPathConstants.Hangfire, new DashboardOptions {
      ...
      app.UseSecurityHeaders(
             SecurityPolicyConfigurations.GetPermissionPolicies())
          .UseHttpsRedirection()
          .UseSwagger()
          ...
          .UseEndpoints(endpoints => {
            ...
            endpoints.MapHealthChecks(); endpoints.MapControllers();
                endpoints.MapHub<ChatHub>(ApiPathConstants.ChatHub);
          });
    }

    // A temporary solution for allowing the HangFire dashboard to work with
    // Docker. At a later stage this class should be replaced with a one that
    // will authorize the user who wants to use the Hangfire dashboard.
